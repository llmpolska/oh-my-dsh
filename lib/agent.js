...
export function activateAgentPlane(ctx, cfg) {
  ...
  async function generateImages(prompt, opts, signal) {
    ...
    // picture back: an image attachment where the API supports it, else SVG
    // markup (which we save as an .svg file).
    const instruction = 'Generate the requested picture now. Return the picture itself: an actual image attachment if your API supports it, otherwise complete SVG markup inside a code block — no prose, no description.'
    ...
    for (const kind of attemptsFor(route.api)) {
      ...
      try {
        ...
        if (kind === 'responses') {
          ...
          body = { model: route.model, prompt: userPrompt, n: 1, size }
        }
      ...
      handler: async (invocation) => {
        ...
        try {
          ...
          if (sub === 'image') {
            ...
            const prompt = arg.replace(/^["']|["']$/g, '')
            const result = await generateImages(prompt, {}, invocation.signal)
