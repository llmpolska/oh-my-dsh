...
export function activateAgentPlane(ctx, cfg) {
  ...
  function statusText(agent) {
    ...
    return [
      ...
      '  image:  ' + c.image.model + (c.image.provider ? ' via ' + c.image.provider : ' @ ' + (c.image.baseUrl || 'custom endpoint')) + ' (key: ' + (c.image.apiKey ? 'inline' : c.image.apiKeyEnv || 'provider-declared') + ', size ' + c.image.size + ', out ./' + c.image.outDir + ')',
  ...
  async function generateImages(prompt, opts, signal) {
    ...
    const size = (opts && opts.size) || route.size || '1024x1024'
    if (!route.apiKey) {
    ...
    for (const kind of attemptsFor(route.api)) {
      ...
      try {
        ...
        let body = { model: route.model, messages: [{ role: 'user', content: prompt }], ...(route.useModalities ? { modalities: ['image', 'text'] } : {}) }
        if (kind === 'responses') {
          url = route.baseUrl + '/responses'
          body = { model: route.model, input: prompt, ...(route.useModalities ? { modalities: ['image', 'text'] } : {}) }
        } else if (kind === 'images') {
          url = route.baseUrl + '/images/generations'
          body = { model: route.model, prompt, n: 1, size }
        }
    ...
    if (candidates.length === 0) {
      ...
      return { ok: false, paths: [], count: 0, message: 'No image came back from ' + route.model + ' via ' + (route.provider || route.baseUrl) + '. Last error: ' + (lastError ? String(lastError.message || lastError) : 'unknown') }
    }
    const paths = []
    let seq = 0
