...
import { EFFORTS, SUBAGENT_POLICIES, loadConfig, viewFromState } from './config.js'
...
export function activateAgentPlane(ctx, cfg) {
  ...
  function visionLocked(agent) {
    return !!(agent && visionLocks.get(agent))
  }
  function resolveTarget(agent, planActiveOverride) {
    const spec = {
      ...
      visionActive: visionLocked(agent),
  ...
  function routeMain(agent, planActiveOverride) {
    ...
    if (visionLocked(agent)) {
      const vis = visionTarget()
      if (vis && cfg.state.visionAuto) ensureHeader(agent, vis)
      return
    }
  ...
  function statusText(agent) {
    ...
    return [
      ...
      '  vision: ' + tierLabel(c.vision) + ' (auto=' + (c.visionAuto ? 'on' : 'off') + ')' + (agent && visionLocked(agent) ? ' — THIS SESSION IS ON THE VISION TIER' : ''),
      '  image:  ' + c.image.model + ' @ ' + c.image.baseUrl + ' (key: ' + (c.image.apiKey ? 'inline' : c.image.apiKeyEnv) + ', size ' + c.image.size + ', out ./' + c.image.outDir + ')',
  ...
  async function applyConfigure(args, agent) {
    if (args.tier === 'vision') {
      ...
      if (agent && visionLocked(agent) && cfg.state.visionAuto) ensureHeader(agent, visionTarget())
    ...
    if (agent && !isChildAgent(agent) && (effectiveMode(agent) === tierName || effectiveMode(agent) === 'auto') && !visionLocked(agent)) ensureHeader(agent, cfg.state[tierName])
    let persisted = ''
  ...
  function collectImageCandidates(value, out) {
    ...
    if (typeof value === 'string') {
      if (value.startsWith('data:image/')) out.push({ kind: 'data', data: value })
      else if (/^https?:\/\//.test(value) && /\.(png|jpe?g|webp|gif)(\?|$)/i.test(value)) out.push({ kind: 'url', url: value })
      return
    }
    if (Array.isArray(value)) { for (const item of value) collectImageCandidates(item, out); return }
    if (typeof value !== 'object') return
    if (typeof value.b64_json === 'string' && value.b64_json.length > 64) { out.push({ kind: 'b64', data: value.b64_json }); return }
    if (value.type === 'image_url' && value.image_url) {
      const u = typeof value.image_url === 'string' ? value.image_url : value.image_url.url
      if (typeof u === 'string') { out.push({ kind: u.startsWith('data:') ? 'data' : 'url', data: u, url: u }); return }
    }
  ...
  /**
   * Generate images through the configured image-capable model. Tries the
   * classic images/generations endpoint first, then a chat-completions call
   * with image output modalities, then the OpenAI Responses API — so a chat
   * model that can emit images (e.g. a "luna"-style GPT) works too.
   */
  async function generateImages(prompt, opts, signal) {
    const c = cfg.state.image
    const key = c.apiKey || (c.apiKeyEnv ? process.env[c.apiKeyEnv] : '') || ''
    const size = (opts && opts.size) || c.size || '1024x1024'
    if (!key) {
      return { ok: false, paths: [], count: 0, message: 'No API key for image generation. Set ' + (c.apiKeyEnv || 'OPENAI_API_KEY') + ' in the environment, or paste a key in the oh my dsh settings tab.' }
    }
    const base = String(c.baseUrl || '').replace(/\/+$/, '')
    if (!base) return { ok: false, paths: [], count: 0, message: 'No image-generation base URL configured.' }
    const auth = { Authorization: 'Bearer ' + key }
    const candidates = []
    let lastError = null
    // 1) classic images/generations
    try {
      const data = await httpJson(base + '/images/generations', {
        method: 'POST', headers: { 'Content-Type': 'application/json', ...auth },
        body: JSON.stringify({ model: c.model, prompt, n: 1, size }),
      }, signal)
      collectImageCandidates(data, candidates)
    } catch (e) { lastError = e }
    // 2) chat completions with image output modalities
    if (candidates.length === 0 && c.useModalities) {
      try {
        const data = await httpJson(base + '/chat/completions', {
          method: 'POST', headers: { 'Content-Type': 'application/json', ...auth },
          body: JSON.stringify({ model: c.model, messages: [{ role: 'user', content: prompt }], modalities: ['image', 'text'] }),
        }, signal)
        collectImageCandidates(data, candidates)
      } catch (e) { if (!lastError) lastError = e }
    }
    // 3) OpenAI Responses API shape
    if (candidates.length === 0) {
      try {
        const data = await httpJson(base + '/responses', {
          method: 'POST', headers: { 'Content-Type': 'application/json', ...auth },
          body: JSON.stringify({ model: c.model, input: prompt, modalities: ['image', 'text'] }),
        }, signal)
        collectImageCandidates(data, candidates)
      } catch (e) { if (!lastError) lastError = e }
    }
    if (candidates.length === 0) {
      return { ok: false, paths: [], count: 0, message: 'No image came back from ' + c.model + '. Last error: ' + (lastError ? String(lastError.message || lastError) : 'unknown') }
    }
    const paths = []
    let seq = 0
    for (const cand of candidates) {
      try {
        ...
        if (bytes) {
          seq = seq + 1
          paths.push(saveImageBytes(bytes, c.outDir, seq))
        }
    ...
    return { ok: true, paths, count: paths.length, message: 'Generated ' + paths.length + ' image(s) with ' + c.model + '.\n' + paths.map((p) => '  ' + p).join('\n') }
  }
  ...
  if (typeof ctx.on === 'function') {
    ...
    ctx.on('agent/inbox/inserted', (payload) => {
      ...
      try {
        ...
        const hasImage = Array.isArray(message.content) && contentHasImage(message.content)
        if (hasImage && cfg.state.visionAuto && visionTarget() && registered(visionTarget().provider)) {
          visionLocks.set(agent, true)
          ensureHeader(agent, visionTarget())
          diag.lastRouting = 'inbox: image message -> vision tier ' + tierLabel(visionTarget()) + ' (' + agentIdentity(agent) + ')'
        } else {
          visionLocks.set(agent, false)
          routeMain(agent)
          diag.lastRouting = 'inbox: routed ' + (hasImage ? '(vision unavailable, tier routing) ' : '') + agentIdentity(agent)
        }
      } catch (e) {
  ...
  if (tools) {
    safeRegister('omd_status tool', () => tools.register(defineTool({
      ...
      async execute(args, exec) {
        ...
        try {
          effective = effectiveExecutionTier(exec.agent) + (visionLocked(exec.agent) ? ' (vision)' : '')
    ...
    safeRegister('omd_image tool', () => tools.register(defineTool({
      name: 'omd_image',
      description: 'Generate images with the configured image-capable model (currently ' + cfg.state.image.model + ' at ' + cfg.state.image.baseUrl + '; configure it in the oh my dsh settings tab). The generated images are saved under ./' + cfg.state.image.outDir + '/ and the absolute paths are returned.',
      parameters: {
        prompt: { type: 'string', required: true, description: 'The image prompt to send to the image model.' },
        size: { type: 'string', description: 'Requested size, e.g. 1024x1024. Default: the configured size.' },
      },
      output: {
        schema: {
          type: 'object',
      ...
      async execute(args, exec) {
        return generateImages(String(args.prompt || ''), { size: args.size }, exec.signal)
      },
    })))
  ...
  if (commands) {
    safeRegister('/omd command', () => commands.register({
      name: 'omd',
      description: 'Control Oh My DSH tiered model routing',
      input: { hint: 'status | strong | cheap | auto | off | plan | models | set <think|build|vision> <provider> <model> [effort] | subagent <inherit|cheap|strong> | vision [on|off] | advisor <question> | review <focus> | image <prompt>' },
      handler: async (invocation) => {
        ...
        try {
          ...
          if (sub === 'strong' || sub === 'think') { const r = await applyRoute('strong', false, invocation.agent); return { kind: 'success', text: r.message } }
          if (sub === 'cheap' || sub === 'build') { const r = await applyRoute('cheap', false, invocation.agent); return { kind: 'success', text: r.message } }
          ...
          if (sub === 'set') {
            ...
            const model = rest[2]
            const effort = rest[3]
            const tierName = tier === 'think' || tier === 'strong' ? 'strong' : tier === 'build' || tier === 'cheap' ? 'cheap' : tier === 'vision' ? 'vision' : undefined
            if (!tierName) return { kind: 'error', text: 'usage: /omd set <think|build|vision> <provider> <model> [off|high|max]' }
            if (!provider || !model) return { kind: 'error', text: 'usage: /omd set <think|build|vision> <provider> <model> [off|high|max]' }
            const r = await applyConfigure({ tier: tierName, provider, model, reasoningEffort: effort, sessionOnly: false }, invocation.agent)
            return { kind: r.ok ? 'success' : 'error', text: r.message }
          }
          if (sub === 'subagent') {
            const policy = rest[0]
            if (SUBAGENT_POLICIES.indexOf(policy) === -1) return { kind: 'error', text: 'usage: /omd subagent <inherit|cheap|strong>' }
            cfg.state.subagentPolicy = policy
            const saved = await persistConfig()
            return { kind: 'success', text: 'Subagent policy set to "' + policy + '".' + (saved ? ' Saved.' : '') }
          }
          if (sub === 'vision') {
            if (rest[0] === 'on' || rest[0] === 'off') {
              cfg.state.visionAuto = rest[0] === 'on'
              const saved = await persistConfig()
              return { kind: 'success', text: 'Automatic vision routing ' + (cfg.state.visionAuto ? 'enabled' : 'disabled') + '.' + (saved ? ' Saved.' : '') }
            }
            const vis = visionTarget()
            return { kind: 'success', text: 'Vision tier: ' + tierLabel(vis) + ' — automatic routing ' + (cfg.state.visionAuto ? 'ON' : 'OFF') + '. When ON, messages containing images run on the vision model. Set it with /omd set vision <provider> <model>.' }
          }
          if (sub === 'plan') {
            cfg.state.mode = 'auto'
            await persistConfig()
            ...
            ensureHeader(invocation.agent, cfg.state.strong)
            return { kind: 'success', text: 'Routing set to auto (think plans, build executes). Think header applied immediately.' + planText }
          }
          if (sub === 'review') {
            if (!arg) return { kind: 'error', text: 'usage: /omd review <focus to review>' }
            const result = await streamText(cfg.state.strong, REVIEW_SYSTEM, 'Review focus:\n' + arg, invocation.signal)
            return { kind: 'success', text: 'Oh My DSH review (' + tierLabel(cfg.state.strong) + '):\n' + result.text }
          }
          if (sub === 'advisor') {
            if (!arg) return { kind: 'error', text: 'usage: /omd advisor <one decision question>' }
            const result = await streamText(cfg.state.strong, ADVISOR_SYSTEM, 'Decision question:\n' + arg, invocation.signal)
            return { kind: 'success', text: 'Oh My DSH advisor (' + tierLabel(cfg.state.strong) + '):\n' + result.text }
          }
          if (sub === 'image') {
            if (!arg) return { kind: 'error', text: 'usage: /omd image <prompt>' }
            const result = await generateImages(arg, {}, invocation.signal)
            return { kind: result.ok ? 'success' : 'error', text: result.message }
          }
          return { kind: 'error', text: 'Unknown subcommand "' + sub + '". Use: status | strong | cheap | auto | off | plan | models | set | subagent | vision | advisor | review | image' }
        } catch (e) {
          return { kind: 'error', text: 'omd command failed: ' + String(e && e.message || e) }
        }
      },
    }))
  }
    ...
    safeRegister('prompt section', () => systemPrompt.section({
      ...
      text: function () {
        ...
        return [
          ...
          '- Messages that contain images automatically route this session to the vision tier; the session returns to its tier routing on the next text message.',
