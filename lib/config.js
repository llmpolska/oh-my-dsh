...
// Vision descriptions, keyed by the sha256 of the image bytes. Written in the
// background by the host half (vision turn wrapper) while the vision model
// answers natively; read by the agent-plane scrub, which replaces image parts
// with the description in requests to text-only models.
export const visionDescriptionStore = new Map()

/** sha256 hex of image bytes — the key the scrub looks up by. */
export function visionImageHash(bytes) {
...
export const omdSchema = z.object({
  ...
  imageModel: z.string().default('gpt-5.6-luna'),
...
export function stateFromView(view) {
  ...
  return {
    ...
    image: {
      provider: v.imageProvider ?? '',
      baseUrl: v.imageBaseUrl ?? '',
      model: v.imageModel ?? 'gpt-5.6-luna',
      apiKeyEnv: v.imageApiKeyEnv ?? '',
