...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const SIZES = ['256x256', '512x512', '1024x1024', '1024x1536', '1536x1024', '1792x1024', '1024x1792']
    ...
    function OhMyDshSection() {
      ...
      return e('div', { style: C.section },
        e('h2', { style: C.title }, 'oh my dsh'),
        e('p', { style: C.intro }, 'Tiered model routing: a think tier plans and reviews, a build tier implements, a vision tier reads images you paste, and an image model generates images. Changes apply to the next step; new sessions use the oh-my-dsh preset.'),
        e(Card, { title: 'Routing', desc: 'How each session picks its model per step.' },
          e('div', { style: C.grid },
            e(Field, { label: 'Mode' },
        ...
        e(Card, { title: 'Vision tier', desc: 'Messages that contain images automatically run on this model.' },
        ...
        e(Card, {
          ...
          desc: 'The model used by the omd_image tool and /omd image. Works with the provider\'s images endpoint, and with chat models that can emit images (e.g. a luna-style GPT) through chat completions with image output modalities.',
        },
          e('div', { style: C.grid },
            e(Field, { label: 'Provider (label)' },
              e('input', { style: C.input, value: draft.imageProvider ?? '', onChange: (ev) => set('imageProvider', ev.target.value) }),
            ),
            e(Field, { label: 'Base URL' },
              e('input', { style: C.input, value: draft.imageBaseUrl ?? '', onChange: (ev) => set('imageBaseUrl', ev.target.value) }),
            ),
            e(Field, { label: 'Image model' },
              e('input', { style: C.input, placeholder: 'e.g. gpt-image-1 or a luna-style GPT id', value: draft.imageModel ?? '', onChange: (ev) => set('imageModel', ev.target.value) }),
            ),
            ...
            e(Field, { label: 'Size' },
              e('input', { style: C.input, list: 'omd-sizes', value: draft.imageSize ?? '', onChange: (ev) => set('imageSize', ev.target.value) }),
            ),
          ...
          e('datalist', { id: 'omd-sizes' }, SIZES.map((s) => e('option', { key: s, value: s }))),
          e('label', { style: C.checkboxRow },
            e('input', { type: 'checkbox', checked: !!draft.imageUseModalities, onChange: (ev) => set('imageUseModalities', ev.target.checked) }),
            'Try chat completions with image output modalities (needed for chat models that generate images)',
          ),
        ),
